...
if [ "$USE_POD_SECURITY_POLICY" = "true" ] ; then
  ...
  # Faras change:
  # UID and GID are readonly variables and can not be overwritten, change the varaible
  # names into UserUID and UserGID
  UserUID=$(echo "$PASSWD" | cut -d':' -f3)
  UserGID=$(echo "$PASSWD" | cut -d':' -f4)
  export USER_UID=$UserUID
  export USER_GID=$UserGID
